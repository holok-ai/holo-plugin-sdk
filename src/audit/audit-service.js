...
class AuditService {
    ...
    async _logRequestToDatabase(request) {
        ...
        const query = `
            INSERT INTO llm_requests (request_id, request_type, model, prompt, options, source_id, user_id, timestamp,
                                      metadata)
    ...
    async _logResponseToDatabase(response) {
        ...
        // Prepare statement with parameterized query for security
        const query = `
            INSERT INTO llm_responses (request_id, response_type, token, model, worker_id, timestamp,
