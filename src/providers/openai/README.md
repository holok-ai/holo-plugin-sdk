...
Full-featured OpenAI provider with support for both Chat Completions and Responses APIs. All types are strongly typed
with no `any` or `unknown`, validated using arktype.
...
**Note:** Translators are fully implemented but not currently used in execution flow. Services work directly with OpenAI
types. Translation layer exists for future Holo format integration.
