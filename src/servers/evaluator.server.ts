...
import { env } from "../env";
import { EvaluatorEvent, AuditServiceEvent, EvaluatorServiceEvent } from '../types/evaluator.types';
...
export class EvaluatorServer extends withQueue(withDB(BaseServer)) {
    ...
    async onInit(): Promise<void> {
        ...
        //      await this.evaluatorService.handleRequest(content as EvaluatorInput);
        //  });
       ...
       // @ts-ignore 
        ...
        const contentEval: EvaluatorServiceEvent = {
         ...
         await this.evaluatorService.handleRequest(contentEval);
         // @ts-ignore 
        const content: EvaluatorEvent = {}
        //     source: "evaluator",
        //     eventName: "source-code-extractor-grader",
        //     timestamp: Date.now(),
        // };
