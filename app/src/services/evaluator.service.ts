...
@injectable()
export class EvaluatorService extends ClassLogger {
    ...
    constructor(
        ...
        super();
        this.log.info('EvaluatorService initialized');
    }
    ...
    async init() {
        ...
        this.log.info(`Loaded ${this.evaluatorRegistry.length} total evaluators`);
    }
    ...
    private async loadExternalEvaluators(): Promise<IEvaluator[]> {
        const logger = this.mlog(this.loadExternalEvaluators);
    ...
    private async queueChainedEvents(savedData: EvaluatorsDataResults | null, evalEvent: EvaluatorEvent): Promise<void> {
        const logger = this.mlog(this.queueChainedEvents);
