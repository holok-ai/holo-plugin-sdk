...
export class HoloTokenService extends ClassLogger {
    ...
    async deactivate(id: string, keyHash: string): Promise<void> {
        await this.holoTokenDB.deactivate(id);
        await this.redisService.del(`${REDIS_KEY_PREFIX}${keyHash}`);
        const authKeys = await this.redisService.keys(`auth:${keyHash}:*`);
        for (const key of authKeys) await this.redisService.del(key);
    }
