...
export class RequestDB extends ClassLogger {
    ...
    async getById(id: string): Promise<ProviderRequest | null> {
        return this.db.queryOne<ProviderRequest>(`SELECT *
    ...
    async listPaginated(filters: {
        ...
        if (filters.access_model) {
            conditions.push(`access_model = $${idx++}`);
        ...
        const [rows, countResult] = await Promise.all([
            this.db.query<ProviderRequest>(`SELECT *
                                            FROM provider_requests ${where}
                                            ORDER BY ${col} ${dir}
                                            LIMIT $${idx++} OFFSET $${idx++}`, [...params, limit, offset]),
            this.db.queryOne<{ count: string }>(`SELECT COUNT(*)::text as count
                                                 FROM provider_requests ${where}`, params),
        ]);
    ...
    async insert(request: Omit<ProviderRequest, 'id'>) {
        ...
        const query = `
            ...
            (organization_id, request_id, application_id, provider_id, protocol_id, user_id, client_identifier,
             access_model, has_tools, thread_id, created_at, metadata, request_raw, system_prompt, last_user_prompt)
        ...
        await this.db.query(query, [
            ...
            !!request_raw ? JSON.stringify(request_raw) : null,
            system_prompt ?? null,
