...
export class CacheController extends BaseController {
    ...
    invalidateProvider = async (req: HoloApiRequest, res: ApiResponse): Promise<void> => {
        try {
            ...
            this.log.info(`Cache invalidation: provider`, {provider_id, org_id, provider_name, caller: req.auth?.userId});
            let deleted = 0;
            ...
            this.handleError(res, error as Error, 'Failed to invalidate provider cache');
        }
    };
    ...
    invalidateApplication = async (req: HoloApiRequest, res: ApiResponse): Promise<void> => {
        try {
            ...
            this.log.info(`Cache invalidation: application`, {application_id, org_id, url_slug, caller: req.auth?.userId});
            let deleted = 0;
            ...
            this.handleError(res, error as Error, 'Failed to invalidate application cache');
        }
    };
    ...
    invalidateAuth = async (req: HoloApiRequest, res: ApiResponse): Promise<void> => {
        try {
            ...
            this.log.info(`Cache invalidation: auth`, {token_hash: token_hash ? `${token_hash.substring(0, 8)}...` : undefined, app_slug, caller: req.auth?.userId});
            let deleted = 0;
            ...
            this.handleError(res, error as Error, 'Failed to invalidate auth cache');
        }
    };
    ...
    invalidateOrg = async (req: HoloApiRequest, res: ApiResponse): Promise<void> => {
        try {
            ...
            if (!org_id) {
                res.status(400).json({success: false, error: {message: 'org_id is required', code: 'VALIDATION_ERROR'}, timestamp: new Date().toISOString()});
                return;
            }
            ...
            this.handleError(res, error as Error, 'Failed to invalidate org cache');
        }
    };
