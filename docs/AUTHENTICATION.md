...
interface Auth {
    ...
    userId?: string;
    tokenType: TokenType;
    application?: Application;    // set when request targets a specific app
    applications: Application[];  // all apps the caller can access
    clientIdentifier?: string;    // forwarded from X-Client-User header
}
```
...
The `X-Client-User` header allows 3rd-party applications to pass their own user identifier for audit purposes. This value is opaque to Holo — it is stored on `Auth.clientIdentifier` and flows through to audit logs without validation.
...
This reduces 3+ roundtrips (token verification, application lookup, access check) to a single Redis GET for repeat requests.
...
JWT auth is cached separately via `TokenService.getAppSlugs()` — no changes from the existing mechanism.
